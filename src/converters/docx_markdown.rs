//! DOCX → Markdown（保留标题、粗体/斜体/删除线、表格、图片、代码块等）。
//!
//! 逻辑移植自 `feishu-export` 的 `doc-converter` crate（XML 遍历 + 关系图嵌入图片）。
...
/// 将 `.docx` 字节转为 Markdown（默认嵌入图片为 data URI）。
pub(crate) fn convert_docx_bytes_to_markdown(bytes: &[u8]) -> Result<String> {
    Converter::new().convert_bytes_inner(bytes)
}
...
struct Converter {
    embed_images: bool,
    output_images_dir: Option<PathBuf>,
}

impl Default for Converter {
    fn default() -> Self {
        Self { embed_images: true, output_images_dir: None }
    }
}
...
impl Converter {
    ...
    fn convert_bytes_inner(&self, bytes: &[u8]) -> Result<String> {
    ...
    fn parse_relationships(&self, zip: &mut ZipArchive<std::io::Cursor<&[u8]>>) -> Result<HashMap<String, String>> {
    ...
    fn extract_plain_text(&self, bytes: &[u8], image_map: &ImageMap) -> Result<String> {
        ...
        for para_start in para_re.find_iter(&xml) {
        ...
        for tcap in table_re.captures_iter(&xml) {
    ...
    fn document_to_md_xml_pass(&self, bytes: &[u8], image_map: &ImageMap, drawing_order: &[String]) -> Result<String> {
        let mut zip = ZipArchive::new(std::io::Cursor::new(bytes))
            .map_err(|e| ConverterError::WordError(format!("zip error: {:?}", e)))?;
