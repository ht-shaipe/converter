...
impl Pdf {
    ...
    pub fn new(
        input: Vec<Token>,
        style: StyleMatch,
        font_config: Option<&super::fonts::FontConfig>,
    ) -> Result<Self, super::MdpError> {
    ...
    ///
    /// If a row causes a page overflow, it is skipped with a warning.
    fn render_table(
        &self,
        doc: &mut Document,
        ...
        // Use a smaller font for tables with many columns to reduce overflow risk
        let table_font_size = if column_count > 6 {
            (self.style.table_cell.size as f32 * 0.7).max(4.0) as u8
        } else if column_count > 4 {
            (self.style.table_cell.size as f32 * 0.85).max(4.0) as u8
        } else {
            self.style.table_cell.size
        };
        let table_header_font_size = if column_count > 6 {
            (self.style.table_header.size as f32 * 0.7).max(4.0) as u8
        } else if column_count > 4 {
            (self.style.table_header.size as f32 * 0.85).max(4.0) as u8
        } else {
            self.style.table_header.size
        };
        ...
        for (i, header_cell) in headers.iter().enumerate() {
            let mut para = genpdfi::elements::Paragraph::default();
            let style = genpdfi::style::Style::new().with_font_size(table_header_font_size);
        ...
        if let Err(_) = header_row.push() {
            warn!("Failed rendering a table header");
        ...
        for (row_idx, row) in rows.iter().enumerate() {
            ...
            for (i, cell_tokens) in row.iter().enumerate() {
                let mut para = genpdfi::elements::Paragraph::default();
                let style = genpdfi::style::Style::new().with_font_size(table_font_size);
            ...
            if let Err(e) = table_row.push() {
                warn!("Failed to push row {} in a table: {}", row_idx, e);
