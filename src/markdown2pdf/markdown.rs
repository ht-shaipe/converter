...
impl Lexer {
    ...
    /// If closing delimiters are not found, falls back to treating the content as plain text.
    fn parse_emphasis(&mut self) -> Result<Token, LexerError> {
        ...
        // Save position after opening delimiters for potential fallback
        let _content_start = self.position;

        let mut content = self.parse_nested_content(|c| c == delimiter, ParseContext::Inline)?;
        content.push(Token::Text(String::from(" ")));

        // Check for proper closing delimiters
        let mut close_count = 0;
        while close_count < level && self.position < self.input.len() && self.current_char() == delimiter {
            close_count += 1;
            self.advance();
        }

        if close_count != level {
            // Unmatched emphasis — fall back to plain text.
            // Reset position to where the opening delimiters started and consume
            // the delimiters as literal text so the parser can continue.
            self.position = start_pos + level; // skip opening delimiters
            let mut text = (0..level).map(|_| delimiter).collect::<String>();
            // Collect content as plain text up to the current position
            while self.position < self.input.len() {
                let ch = self.current_char();
                if ch == '\n' || self.is_start_of_special_token(ParseContext::Inline) {
                    break;
                }
                text.push(ch);
                self.advance();
